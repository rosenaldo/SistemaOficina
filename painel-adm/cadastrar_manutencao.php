...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            <i class="fas fa-tools mr-2"></i>Tipos PCM
        </h1>
        <a href="index.php?pag=<?php echo $pag ?>&funcao=novo"
            class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">
            <i class="fas fa-plus fa-sm text-white-50"></i> Novo Tipo PCM
        </a>
    </div>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 bg-primary text-white">
            <h6 class="m-0 font-weight-bold">Lista de Tipos PCM</h6>
        </div>
        <div class="card-body">
        ...
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                ...
                if (@$_GET['funcao'] == 'editar') {
                    $titulo = "Editar Tipo PCM";
                    $id2 = $_GET['id'];

                    $query = $pdo->query("SELECT * FROM tipo_pcm WHERE id = '$id2'");
                    $res = $query->fetchAll(PDO::FETCH_ASSOC);
                    $nome2 = $res[0]['descricao'];
                } else {
                    $titulo = "Novo Tipo PCM";
                }
                ?>
            ...
            <form id="form" method="POST">
                <div class="modal-body">
                    <div class="form-group">
                        <label for="nome_mec">Descrição</label>
                        <input value="<?php echo @$nome2 ?>" type="text" class="form-control" id="nome_mec" name="nome_mec" 
                               placeholder="Digite a descrição do tipo PCM" required>
                    </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header bg-danger text-white">
                <h5 class="modal-title" id="modal-deletarLabel">Excluir Tipo PCM</h5>
                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <p>Tem certeza que deseja excluir este tipo PCM permanentemente?</p>
